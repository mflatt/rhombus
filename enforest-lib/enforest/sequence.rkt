  ...
  (syntax-parse stx
    [(_ (~alt (~optional (~seq #:syntax-class form:id)
     ...
     #`(begin
         ...
         (define (apply-transformer head head-name head-tail tail-tail . env)
...
(define (apply-sequence-transformer env t orig-head id stx tail track-origin use-site-scopes? checker)
