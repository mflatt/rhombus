...
@doc(
 ...
 Converts @rhombus(tm) to a string format @tt{HH@litchar{:}MM} (if
 @rhombus(time_format) is @rhombus(#'minutes)),
 @tt{HH@litchar{:}MM@litchar{:}SS} (if @rhombus(time_format) is at least
 @rhombus(#'seconds)), @tt{HH@litchar{:}MM@litchar{:}SS@litchar{.}SSS},
 @tt{HH@litchar{:}MM@litchar{:}SS@litchar{.}SSSSSS},
 @tt{HH@litchar{:}MM@litchar{:}SS@litchar{.}SSSSSSSSS}, depending on the
 precision needed to report the nanoseconds of @rhombus(tm).
...
@examples(
  ...
  ~repl:
    ...
    tm.to_string()
    tm.to_string(~time_format: #'nanoseconds)
)
...
@doc(
 ...
 the string includes the time portion of @rhombus(dt). If
 @rhombus(time_format) is @rhombus(#'nanoseconds), then the time portion
 can include nanoseconds if supported by the format. If a format requires
 a time zone, then UTC is used. Depending on @rhombus(format),
 @rhombus(dt) may be converted to a @rhombus(date.ZonedDateTime) to
 produce the string.
...
@doc(
 ...
 The @rhombus(#'nanoseconds) format normally shows nanoseconds only when
 non-zero.
