...
in its @tech{scope set}. As a special case, the @tech{expression} space
...
not in the current @tech{local binding context}, it throws a syntax error.
...
is used; otherwise, the expander throws a syntax error.
...
@itemlist(
 ...
 @item{A transformer definition form like @rhombus(expr_meta.macro): The right-hand side is
...
the name of the procedure. In this case, Rhombus decides, at
...
Terms still needing definition: @deftech{transformer}, @deftech{base phase},
@deftech{syntactic form}, @deftech{expression}, @deftech{channel}, @deftech{inspector},
@deftech{source location}, @deftech{syntax property}, @deftech{will},
@deftech{module context}, @deftech{module registry}, @deftech{top level context},
@deftech{partial expansion}, @deftech{namespace}.

@subsection(~tag: "intro-binding"){Section to Appear}
@subsection(~tag: "modprotect"){Section to Appear}
