...
@itemlist(
 ...
 be evaluated once per occurrence of the binding. If a binding is used
 repeating @rhombus(..., ~bind), for example, matching will be applied
 multiple times, but one-time definitions are evaluated just once to use
 for all of those match attempts. Typically, one-time code corresponds to
 an expression within a binding form, such as expressions in
 @rhombus(Any.of). The matcher step can directly refer to definitions
 generated by the oncer step.}
