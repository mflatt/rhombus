...
(DC), which is an instance of the @rhombus(draw.DC, ~annot) interface.
For example, the @rhombus(draw.PDFDC, ~class) class implements
@rhombus(draw.DC, ~annot) for drawing to a PDF file, while
@rhombus(draw.Bitmap.make_dc) produces @rhombus(draw.DC, ~annot)
...
@rhombus(draw.DC, ~annot) instance for drawing into the canvas window.

@// ------------------------------------------------------------
@section{Lines and Simple Shapes}

To draw into a bitmap, first create the bitmap with
@rhombus(draw.Bitmap), and then calls its @rhombus(draw.Bitmap.make_dc)
...
Use methods like @rhombus(draw.DC.line) on the DC to draw into the
...
A line-drawing drawing operation like @rhombus(draw.DC.line) uses the
DC's current @defterm{pen} to draw the line, which can be accessed or
changed as the @rhombus(draw.DC.pen) property. A pen has a color, line
width, and style, where pen styles include @rhombus(#'solid),
@rhombus(#'long_dash), and @rhombus(#'transparent). Enclosed-shape
operations like @rhombus(draw.DC.rectangle) use both the current pen and
the DC's current @deftech{brush}, which is access or set as
@rhombus(draw.DC.brush). A brush has a color and style, where brush
...
equivalently, use @rhombus(draw.Pen.none):
...
@rhombus(draw.DC.pen), @rhombus(draw.DC.brush), or other properties of
the DC, and normally it should restore settings afterward. Use
@rhombus(draw.DC.save) to push the current state to a internal stack,
and use @rhombus(draw.DC.restore) to pop the stack and restore that
state---or use the @rhombus(draw.DC.save_and_restore) form to wrap
@rhombus(draw.DC.save) and @rhombus(draw.DC.restore) around a body
sequence.
...
transformed further with methods like @rhombus(draw.DC.scale),
@rhombus(draw.DC.translate), or @rhombus(draw.DC.rotate):
...
Drawing functions like @rhombus(draw.DC.line) and
@rhombus(draw.DC.rectangle) are actually convenience functions for the
more general @rhombus(draw.DC.path) operation. The
@rhombus(draw.DC.path) operation takes a @deftech{path}, which describes
...
An instance of @rhombus(draw.Path) holds a path. Conceptually, a path
has a current pen position that is manipulated by methods like
@rhombus(draw.Path.move_to), @rhombus(draw.Path.line_to), and
@rhombus(draw.Path.curve_to). The @rhombus(draw.Path.move_to) method
starts a subpath, and @rhombus(draw.Path.line_to) and
@rhombus(draw.Path.curve_to) extend it. The @rhombus(draw.Path.close)
...
that can be filled with the brush. A @rhombus(draw.Path) object can have
...
@rhombus(draw.Path.close), then all of the corners are joined, including
...
Draw text using the @rhombus(draw.DC.text) method, which takes a string
...
font is described by a @rhombus(draw.Font) object and installed as the
@rhombus(draw.DC.font) property. The color of drawn text, which is
separate from either the pen or brush, can be set via the
@rhombus(draw.DC.text_foreground) property.
...
@rhombus(draw.DC.text_extent), which returns four values: the total
...
result of @rhombus(draw.DC.text_extent) can be used to position text
...
When you create or @rhombus(draw.DC.clear) a bitmap, the content is
nothing. ``Nothing'' isn't the same as white; it's the absence of
drawing. For example, if you take @rhombus(text_target) from the
previous section and copy it onto another DC using
@rhombus(draw.DC.bitmap), then the black rectangle and blue text is
...
loaded with @rhombus(draw.Bitmap.from_file) preserve transparency in the
image file through the bitmap's alpha channel.

An alpha channel isn't all or nothing. When the edges text is
anti-aliased by @rhombus(draw.DC.text), for example, the pixels are
...
@rhombus(draw.DC.bitmap) causes the blue and black of the ``Hello,
...
setting @rhombus(draw.DC.alpha) affects individual steps and the way
...
Use @rhombus(draw.DC.using_alpha) to apply alpha compositing to a
sequence of drawing commands, rendering them all at once with the given
opacity. The @rhombus(draw.DC.using_alpha) form wraps
@rhombus(draw.DC.start_alpha) and @rhombus(draw.DC.end_alpha) around its
body.
...
@rhombus(draw.DC.bitmap).

Combining regions with @rhombus(draw.Brush) objects that have gradients,
...
different classes that implement @rhombus(draw.DC, ~class), or different
...
@itemlist(
 ...
 @rhombus(draw.Bitmap.make_platform) uses platform-specific drawing
 operations as much as possible. Depending on the platform, however, a
 bitmap produced by @rhombus(draw.Bitmap.make_platform) may have no alpha
 ...
 draw into the result of a @rhombus(draw.Bitmap.make_platform) call and then copy
 ...
 @rhombus(draw.Bitmap.make_platform), but it retains constrained resources only
 ...
 drawing as @rhombus(draw.Bitmap.make_platform) on Windows or Mac OS, but
