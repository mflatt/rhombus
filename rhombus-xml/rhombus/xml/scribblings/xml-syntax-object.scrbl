...
@itemlist(
 ...
 @item{@rhombus(string): Corresponds to @rhombus(xml.PCData, ~class).}

 @item{@rhombus(&, ~datum): Corresponds  to @rhombus(xml.Entity, ~class).}

 @item{@rhombus(other): Any value that is satisfies
  @rhombus(Content, ~annot). For example, a @rhombus(xml.Comment, ~class)
  value can appear here. Constructing such objects will require a
  @rhombus($) escape.}
